...
//
// "fragColor", "fragment coordinate" is an input variable.
//
// It tells us at which pixel we are on the screen. The coordinate center
// is the left bottom corner, and coordinate values increadse towards
// right and upwards.
//
// The main function is run for each and every pixel on the screen.
// At each call the "gl_FragCoord" has the coordinates of the corresponding pixel.
//
...
// calculations are done in parallel and it is impossible to know which one
// will finish before the other one)
// The outcome of a pizel can only depend on the pixel coordinate (and some
// other input variables.)
// This is the most important difference of shader programming. We'll come
// to this point again and again.
